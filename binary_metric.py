...
def hausdorff_distance(image0, image1):
    ...
    >>> points_b = (6, 0)
    >>> shape = (7, 1)
    >>> image_a = numpy.zeros(shape, dtype=bool)
    >>> image_b = numpy.zeros(shape, dtype=bool)
    >>> image_a[points_a] = True
    >>> image_b[points_b] = True
    >>> hausdorff_distance(image_a, image_b)
    3.0
    """
    a_points = numpy.transpose(numpy.nonzero(image0))
    b_points = numpy.transpose(numpy.nonzero(image1))

    # Handle empty sets properly:
    # - if both sets are empty, return zero
    # - if only one set is empty, return infinity
    if len(a_points) == 0:
        return 0 if len(b_points) == 0 else numpy.inf
    elif len(b_points) == 0:
        return numpy.inf
