...
# path
input_path = r'F:/DELINEATE CAD/ConvMixUnet2_5D_3/predictions_test_60/pred.hdf5'
output_path = r'...'
# read data
...
for i in range(len(data['img_raw'])):    
    ...
    img_raw = cv2.normalize(src=data['img_raw'][i], dst=None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX,
                            dtype=cv2.CV_8U)
    dim = img_raw.shape[0]
    pred = data['mask'][i].astype(np.uint8)
    ...
    if c == '1' or c == '2':
        while(1):      
            print('1: add seg; 2: canc seg; 5: esc')
            c = input("Enter a command: ")
            print(c)
...
hdf5_file = h5py.File(os.path.join(output_path, 'post_proc.hdf5'), "w")
